...
# FIXED: duplicates are retained for management instead of being removed on append/upload
# =============================================================
...
# -----------------------------
# IMPROVED KEY NORMALIZATION (kept close to your working approach)
# -----------------------------
...
calculate_duplicate_confidence <- function(row1, row2, key_cols = duplicate_key_cols) {
  ...
  for (col in key_cols) {
    ...
    if (!is.na(val1) && !is.na(val2) && val1 != "" && val2 != "") {
      ...
      norm1 <- if (col == "SIA_date") .format_sia_key(val1) else .clean_key_vec(val1)
      norm2 <- if (col == "SIA_date") .format_sia_key(val2) else .clean_key_vec(val2)
...
find_partial_duplicates <- function(df, threshold = 85, key_cols = duplicate_key_cols) {
  ...
  for (i in 1:(nrow(reps) - 1)) {
    for (j in (i + 1):nrow(reps)) {
      conf <- calculate_duplicate_confidence(reps[i, , drop = FALSE], reps[j, , drop = FALSE], key_cols)
...
# -----------------------------
# FAST KEY_ID (same approach as previous version)
# -----------------------------
...
# exact duplicate finder kept in the same style as previous working version
...
server <- function(input, output, session) {
  ...
  observeEvent(input$append_manual_rows, {
    ...
    # IMPORTANT FIX:
    # keep duplicates for duplicate management instead of removing them here
    save_repo(repo)
    admin_repo(repo)
  ...
  output$duplicate_panel <- renderUI({
    ...
    div(
      ...
      div(style = "margin-bottom: 10px;",
          actionButton("find_partial_duplicates", "Find Partial Duplicates", class = "btn-default", style = "font-size: 12px;")
      ),
  ...
  output$partial_duplicates_table <- renderDT({
    ...
    partials <- find_partial_duplicates(repo, threshold = input$dup_confidence_threshold)
    ...
    build_sample <- function(kid) {
      rows <- repo[repo$KEY_ID == kid, , drop = FALSE]
      if (nrow(rows) == 0) return("N/A")
      
      cols <- intersect(c("Country", "Province", "District", "SIA_date", "Round_Add", "Vaccine_type", "Response"), names(rows))
      paste(paste(cols, as.character(unlist(rows[1, cols, drop = FALSE])), sep = ": "), collapse = " | ")
    }
    
    partials$sample_data_1 <- vapply(partials$KEY_ID_1, build_sample, character(1))
    partials$sample_data_2 <- vapply(partials$KEY_ID_2, build_sample, character(1))
